    ...
    public static class Description
    {
        public static void Add(DataTable table, MTConnectDevices.Device device)
        {
            if (device.Description != null)
            {
                ...
                {
                    string logoUrl = new Uri("https://images.jbook.in/device-image?manufacturer=" + device.Description.Manufacturer).ToString();
                    DeviceConfiguration.EditTable(table, "/Description/LogoUrl", logoUrl, null);

                    if (!string.IsNullOrEmpty(device.Description.Model))
                    {
                        string imageUrl = new Uri("https://images.jbook.in/device-image?manufacturer=" + device.Description.Manufacturer + "&model=" + device.Description.Model).ToString();
